...
pub fn format_open_bets(views: &[OpenBetView], compact: bool) -> String {
    ...
    for v in views {
        ...
        let strat_label = strategy_label(&bet.strategy);
        ...
        if compact {
            ...
            let source_icon = source_icon(&bet.source);
...
pub fn format_stats(data: &StatsData) -> String {
    ...
    let strat_lines: Vec<String> = data
        ...
        .iter()
        .map(|s| {
            let label = strategy_label(&s.name);
    ...
    for src in &data.sources {
        let label = source_icon(&src.name);
...
mod tests {
    ...
    fn test_stats_roi_uses_pnl_not_bankroll_diff() {
        let data = make_stats(1000.0, 800.0, 50.0);
