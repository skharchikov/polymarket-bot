...
/// currently open bets. Returns one decision per candidate, defaulting to
/// `keep = true` on any error or when there are no open bets to compare against.
pub(super) async fn portfolio_correlation_check(
    ...
    if candidates.is_empty() || open_bets.is_empty() {
        return candidates
            .iter()
