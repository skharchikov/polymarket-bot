...
impl PgPortfolio {
    ...
    /// Return `last_checked_at` for the given trader wallet, if set.
    #[allow(dead_code)]
    pub async fn last_trader_check(&self, wallet: &str) -> Result<Option<DateTime<Utc>>> {
        let row: Option<(Option<DateTime<Utc>>,)> =
            sqlx::query_as("SELECT last_checked_at FROM followed_traders WHERE proxy_wallet = $1")
                .bind(wallet)
                .fetch_optional(&self.pool)
                .await
                .context("last_trader_check")?;
...
impl FollowedTraderRow {
    fn into_trader(self) -> FollowedTrader {
        FollowedTrader {
            id: self.id,
